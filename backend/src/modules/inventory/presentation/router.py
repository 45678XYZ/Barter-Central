...
async def create_item(
    ...
    category: ItemCategory = Form(
        ..., description="物品分類 (TEXTBOOK, 3C, DAILY, OTHER)"
    ),
    # 使用 File(...) 來接收圖片檔案
    ...
    if image.content_type not in [
        ...
        raise HTTPException(
            status_code=400,
            detail="Only JPEG or PNG or heic or webp images are allowed.",
        )
...
# ---------------------------------------------
@router.get("/", response_model=List[ItemResponse], summary="搜尋物品列表")
def search_items(
...
# ---------------------------------------------
@router.get(
    "/me", response_model=List[ItemResponse], summary="取得我的物品清單 (含歷史紀錄)"
)
def get_my_items(
    current_user: User = Depends(get_current_user),
    service: ItemService = Depends(get_item_service),
...
# ---------------------------------------------
@router.get("/categories", response_model=List[dict], summary="取得物品分類清單")
def get_categories():
    """
    ...
    return [
        {"id": category.value, "name": CATEGORY_MAP.get(category, category.value)}
...
# ---------------------------------------------
@router.get("/{item_id}", response_model=ItemResponse, summary="取得特定物品詳情")
